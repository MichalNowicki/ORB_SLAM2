...
#ifndef ORB_SLAM2_EDGESE3PROJECTXYZPATCH_H
#define ORB_SLAM2_EDGESE3PROJECTXYZPATCH_H
...
namespace g2o {
    ...
    class EdgeProjectXYZPatch : public g2o::BaseMultiEdge<9, Vector9D> {
    public:
        ...
        void setAdditionalData(std::vector<double> _largePatchA, std::vector<double> _largePatchB,
                               double _pointScaleA, double _pointScaleB) {
            largePatchA = _largePatchA;
            largePatchB = _largePatchB;
            pointScaleA = _pointScaleA;
            pointScaleB = _pointScaleB;

            int largePatchElements = largePatchA.size();

            largePatchStride = sqrt(largePatchElements);
            largePatchCenter = (largePatchStride - 1) / 2;


            // TODO: Compute gradients for analytical jacobians
        }
...
//        virtual void linearizeOplus();
        ...
        double getPatchValue(double projU, double projV, double measU, double measV,
                             double addU, double addV, double pointScale, std::vector<double> &patch);
...
//        inline Matrix<double, 3, 1, Eigen::ColMajor> d_Tinvpsi_d_psi(const SE3Quat &T, const Vector3D &psi);
    ...
    private:
        ...
        std::vector<double> largePatchA, largePatchB;
//        std::vector<Vector2D> largePatchObsGradient;
        double pointScaleA, pointScaleB;
        int largePatchCenter, largePatchStride;
    };
}
#endif //ORB_SLAM2_EDGESE3PROJECTXYZPATCH_H
