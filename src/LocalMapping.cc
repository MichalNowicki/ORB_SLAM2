                    ...
                    if (mpMap->KeyFramesInMap() > 2) {
                        ...
                        if (optimizationType == 0) {
                            ...
                            chi2 = OptimizerBA::LocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, mpMap,
                                                                      OptimizerBA::TYPE::INVERSE_DEPTH, baCounter,
                                                                      sigma);
                            baCounter++;
                        }
                        ...
                        auto diff = end - start;
                        cout << "\tLocalMapping time: " << chrono::duration<double, milli>(diff).count() << " ms"
                             << endl;
...
//    chi2statistics = Optimizer::GlobalBundleAdjustemnt(mpMap,10,&mbStopGBA,mpCurrentKeyFrame->mnId,false, sigma);
        chi2statistics = OptimizerBA::GlobalBundleAdjustment(mpMap, OptimizerBA::TYPE::INVERSE_DEPTH_SINGLE_PARAM_PATCH,
                                                             sigma);
        /*
        ...
        */
        ...
        std::cout << "\tAvg reprojection error : "
                  << accumulate(reprojectionErr.begin(), reprojectionErr.end(), 0.0) / reprojectionErr.size()
                  << std::endl;
...
//    std:: cout << "Fuse added new measurements: " << densifyCount << std::endl;
    ...
    void LocalMapping::SetFinish() {
        ...
        std::ofstream chi2stream("GBA_chi2.txt"), reprojstream("GBA_reproj.txt"), keyframeCountAndTimeDiffStr(
                "LBA_keyframeCountAndTimeDiff.txt");
